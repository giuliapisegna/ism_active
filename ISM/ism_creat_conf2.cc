...
void confinedcenter_position(glsim::OLconfiguration &conf, scomp &SC)
{
  conf.N=SC.N;
  conf.step=0;
  conf.time=0;
...
//metto le particelle in una posizione random in una scatola posta al centro della scatola originaria determinata dalla densita' e che e' 10 volte piu' piccola di quella originaria
  glsim::Uniform_real ranx(conf.box_length[0]/2 -conf.box_length[0]/20, conf.box_length[0]/2 + conf.box_length[0]/20);
  glsim::Uniform_real rany(conf.box_length[1]/2 -conf.box_length[1]/20, conf.box_length[1]/2 + conf.box_length[1]/20);
  glsim::Uniform_real ranz(conf.box_length[2]/2 -conf.box_length[2]/20, conf.box_length[2]/2 + conf.box_length[2]/20);
...
CLoptions::CLoptions() : UtilityCL("ism_greate_conf")
{
  ...
  command_line_options().add_options()
    ...
    ("confinecenter,CC",po::bool_switch(&options.confinecenter)->default_value(false),
     "confine the particles in a box with dimensions L/10 put at the center of the real box")
    ;
...
void wmain(int argc,char *argv[])
{
  ...
  else if(options.confinecenter)
     confinedcenter_position(conf,SC);
  else
